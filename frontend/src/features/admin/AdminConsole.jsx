...
export default function AdminConsole({ profile }) {
  ...
  const pages = {
    ...
    operations: <OperationsPage key={organizationId} api={api} canManage={isSuperadmin || Boolean(entitlements.queues)} />,
    hours: <HoursPage config={config} setConfig={setConfig} onSave={() => saveConfig()} />, handling: <HandlingPage config={config} setConfig={setConfig} extensions={extensions} onSave={() => saveConfig()} />, reports: <ReportsPage key={organizationId} api={api} />, events: <EventsPage events={events} />,
